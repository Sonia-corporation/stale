...
describe(`IssuesInputsService`, (): void => {
  ...
  describe(`setInputs()`, (): void => {
    ...
    let coreInputsServiceGetEnumInputSpy: jest.SpyInstance;

    beforeEach((): void => {
      service.inputs$$ = createHydratedMock<IIssuesInputs>(<IIssuesInputs>{
        issueAddLabelsAfterClose: [`extra-added-close-label-1`, `extra-added-close-label-2`],
        issueAddLabelsAfterStale: [`extra-added-stale-label-1`, `extra-added-stale-label-2`],
      ...
      coreInputsServiceGetEnumInputSpy = jest
        .spyOn(CoreInputsService, `getEnumInput$$`)
        .mockImplementation((_name: string, _callback: unknown, fallback: unknown): unknown => fallback);
    });
    ...
    it(`should get the issue-close-reason input, parse it and set it`, (): void => {
      expect.assertions(3);

      service.setInputs();

      expect(coreInputsServiceGetEnumInputSpy).toHaveBeenCalledTimes(1);
      expect(coreInputsServiceGetEnumInputSpy).toHaveBeenNthCalledWith(
        1,
        `issue-close-reason`,
      ...
      expect(service.inputs$$?.issueCloseReason).toBe(ECloseReason.NOT_PLANNED);
    });
    ...
    it(`should get the issue-only-with-milestones input, parse it and set it`, (): void => {
      expect.assertions(3);

      service.setInputs();

      expect(coreGetBooleanInputSpy).toHaveBeenCalledTimes(8);
      expect(coreGetBooleanInputSpy).toHaveBeenNthCalledWith(6, `issue-only-with-milestones`, { required: false });
    ...
    it(`should get the issue-processing input, parse it and set it`, (): void => {
      ...
      expect(coreGetBooleanInputSpy).toHaveBeenCalledTimes(8);
      expect(coreGetBooleanInputSpy).toHaveBeenNthCalledWith(8, `issue-processing`, { required: false });
      expect(service.inputs$$?.issueProcessing).toBeFalse();
    });

    it(`should get the issue-remove-labels-after-close input, parse it and set it`, (): void => {
      expect.assertions(3);

      service.setInputs();

      expect(coreGetMultilineInputSpy).toHaveBeenCalledTimes(11);
      expect(coreGetMultilineInputSpy).toHaveBeenNthCalledWith(10, `issue-remove-labels-after-close`, {
        required: false,
      });
      expect(service.inputs$$?.issueRemoveLabelsAfterClose).toStrictEqual([
        `dummy-issue-remove-labels-after-close-1`,
        `dummy-issue-remove-labels-after-close-2`,
      ]);
    });

    it(`should get the issue-remove-labels-after-stale input, parse it and set it`, (): void => {
      expect.assertions(3);

      service.setInputs();

      expect(coreGetMultilineInputSpy).toHaveBeenCalledTimes(11);
      expect(coreGetMultilineInputSpy).toHaveBeenNthCalledWith(11, `issue-remove-labels-after-stale`, {
        required: false,
      });
      expect(service.inputs$$?.issueRemoveLabelsAfterStale).toStrictEqual([
        `dummy-issue-remove-labels-after-stale-1`,
        `dummy-issue-remove-labels-after-stale-2`,
      ]);
    });
  ...
  describe(`logInputs()`, (): void => {
    ...
    describe(`when the inputs are set`, (): void => {
      beforeEach((): void => {
        service.inputs$$ = createHydratedMock<IIssuesInputs>(<IIssuesInputs>{
          issueAddLabelsAfterClose: [`dummy-extra-added-close-label-1`, `dummy-extra-added-close-label-2`],
          issueAddLabelsAfterStale: [`dummy-extra-added-stale-label-1`, `dummy-extra-added-stale-label-2`],
          issueCloseComment: `dummy-issue-close-comment`,
          issueCloseReason: ECloseReason.NOT_PLANNED,
          issueDaysBeforeClose: 666,
          issueDaysBeforeStale: 666,
      ...
      it(`should log the issue add labels after close input`, (): void => {
        ...
        expect(loggerServiceInfoSpy).toHaveBeenCalledTimes(28);
      ...
      it(`should log the issue close reason input`, (): void => {
        expect.assertions(4);
        ...
        expect(loggerServiceInfoSpy).toHaveBeenNthCalledWith(
          4,
          `white-├──`,
          `input-issue-close-reason`,
          `value-not planned`
        );
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(4, `issue-close-reason`);
      });
      ...
      it(`should log the issue days before close input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(5, `issue-days-before-close`);
      });
      ...
      it(`should log the issue days before stale input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(6, `issue-days-before-stale`);
      });
      ...
      it(`should log the issue ignore all assignees input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(7, `issue-ignore-all-assignees`);
      });
      ...
      it(`should log the issue ignore all labels input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(8, `issue-ignore-all-labels`);
      });
      ...
      it(`should log the issue ignore any assignees input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(11, `issue-ignore-any-assignees`);
      });
      ...
      it(`should log the issue ignore any labels input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(12, `issue-ignore-any-labels`);
      });
      ...
      it(`should log the issue ignore before creation date input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(15, `issue-ignore-before-creation-date`);
      });
      ...
      it(`should log the issue limit api mutations count input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(16, `issue-limit-api-mutations-count`);
      });
      ...
      it(`should log the issue limit api queries count input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(17, `issue-limit-api-queries-count`);
      });
      ...
      it(`should log the issue only any assignees input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(18, `issue-only-any-assignees`);
      });
      ...
      it(`should log the issue only with assignees input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(21, `issue-only-with-assignees`);
      });

      it(`should log the issue only with milestones input`, (): void => {
        expect.assertions(4);
      ...
      it(`should log the issue processing input`, (): void => {
        ...
        expect(loggerServiceInfoSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInfoSpy).toHaveBeenNthCalledWith(24, `white-├──`, `input-issue-processing`, `value-false`);
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(24, `issue-processing`);
      });

      it(`should log the issue remove labels after close input`, (): void => {
        expect.assertions(4);
        ...
        expect(loggerServiceInfoSpy).toHaveBeenNthCalledWith(
          25,
          `white-├──`,
          `input-issue-remove-labels-after-close`,
          `value-dummy-extra-removed-close-label-1,dummy-extra-removed-close-label-2`
        );
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(25, `issue-remove-labels-after-close`);
      });

      it(`should log the issue remove labels after stale input`, (): void => {
        expect.assertions(4);
        ...
        expect(loggerServiceInfoSpy).toHaveBeenNthCalledWith(
          26,
          `white-├──`,
          `input-issue-remove-labels-after-stale`,
          `value-dummy-extra-removed-stale-label-1,dummy-extra-removed-stale-label-2`
        );
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(26, `issue-remove-labels-after-stale`);
      });
      ...
      it(`should log the issue stale comment input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(27, `issue-stale-comment`);
      });
      ...
      it(`should log the issue stale label input`, (): void => {
        ...
        expect(loggerServiceInputSpy).toHaveBeenCalledTimes(28);
        expect(loggerServiceInputSpy).toHaveBeenNthCalledWith(28, `issue-stale-label`);
      });
    });
