---
id: pull-request-close-comment-option
title: Close pull request comment option
tags:
  - Pull requests
  - Options
---

### Input {#input}

Name: `pull-request-close-comment`  
Type: `string`  
Default value: `''`

### Description {#description}

This option will let you define the comment that will be added on the pull requests when they are closed after being stale for too long during the processing.  
This can be useful if you wish to improve the communication and the process on how to handle an unwanted closed pull request.

### Example {#example}

```yml {6}
# ...
name: Stale
id: stale
uses: sonia-corporation/stale@latest
with:
  pull-request-close-comment: |
    This pull request is now closed due to a lack of activity!
```
